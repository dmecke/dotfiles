...
bred='\033[1;38;2;255;60;60m'
gray='\033[38;2;150;150;150m'
dimgray='\033[38;2;90;90;90m'
dim='\033[2m'
reset='\033[0m'
...
color_for_ctx() {
    local t=$1
    if   [ "$t" -ge 300000 ]; then printf "$bred"
    elif [ "$t" -ge 250000 ]; then printf "$red"
    elif [ "$t" -ge 200000 ]; then printf "$yellow"
    elif [ "$t" -ge 120000 ]; then printf "$orange"
    else printf "$green"
    fi
}
...
build_bar() {
    ...
    printf "${white}${filled_str}${dimgray}${empty_str}${reset}"
}
...
format_epoch_time() {
    ...
    case "$style" in
        ...
        datetime)
            ...
            ;;
        relative)
            local now_e diff h m
            now_e=$(date +%s)
            diff=$(( epoch - now_e ))
            [ "$diff" -lt 0 ] && diff=0
            h=$(( diff / 3600 )); m=$(( (diff % 3600) / 60 ))
            if [ "$h" -gt 0 ]; then result="${h}hr ${m}min"; else result="${m}min"; fi
            ;;
        weekday)
            result=$(LC_ALL=C date -j -r "$epoch" +"%a %H:%M" 2>/dev/null)
            [ -z "$result" ] && result=$(LC_ALL=C date -d "@$epoch" +"%a %H:%M" 2>/dev/null)
...
ctx_fmt=$(awk -v t="$current" 'BEGIN{printf "%.1fk", t/1000}')
ctx_color=$(color_for_ctx "$current")
...
line1+="${sep}"
line1+="${dim}ctx${reset} ${ctx_color}${ctx_fmt}${reset}"
line1+="${sep}"
line1+="${skip_perms}${gray}${dirname}${reset}"
if [ -n "$git_branch" ]; then
    line1+=" ${gray}(${git_branch}${git_dirty})${reset}"
fi
...
if [ -n "$five_hour_pct" ]; then
    five_hour_reset=$(format_epoch_time "$five_hour_reset_epoch" "relative")
    five_hour_bar=$(build_bar "$five_hour_pct" "$bar_width")
    five_hour_pct_color="$gray"
...
if [ -n "$seven_day_pct" ]; then
    seven_day_reset=$(format_epoch_time "$seven_day_reset_epoch" "weekday")
    seven_day_bar=$(build_bar "$seven_day_pct" "$bar_width")
    seven_day_pct_color="$gray"
